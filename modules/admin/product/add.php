...
if (isset($_POST['btn_pro'])) {
    ...
    if (empty($_POST['price-og']))
    $error['price-og'] = "không được để trống giá sản phẩm";
else {
        $price_og = $_POST['price-og'];
}
    if (empty($_POST['code']))
    ...
    if (empty($error)) {
        $sql = "INSERT INTO `dhd_product` (`cat_id`, `product_name`, `price`,`price-og`, `code`, `desc`, `thumb`, `product_show`)"
                ."VALUES ('{$cat_id}', '{$product_name}', '{$price}', '{$price_og}', '{$code}', '{$desc}', '{$thumb}', '{$product_show}')";
        if(mysqli_query($conn, $sql))
        echo "<script>window.location.href='?mod=home&act=admin';
        alert('thành công');
       </script>";
        } else {
            echo "<script>window.location.href='?mod=product&act=add';
            alert('thất bại');
           </script>";
        }
    }
        ...
        <div class="col-lg-6 col-md-6">
            <div class="account_form">
                ...
                <form enctype="multipart/form-data" action="" method="POST">
                    ...
                    <p>
                        <label>giá bán sản phẩm<span>*</span></label>
                        <input type="int" name="price" autocomplete="off" /><br>
                        <?php echo form_error('price'); ?>

                    </p>
                    <p>
                        <label>giá gốc sản phẩm<span>*</span></label>
                        <input type="int" name="price-og" autocomplete="off" /><br>
                        <?php echo form_error('price-og'); ?>
